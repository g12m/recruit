...
class Cmf extends TagLib
{
    ...
    protected $tags = [
        ...
        'links'               => ['attr' => 'id', 'close' => 1],//非必须属性item
        ...
        'navcats'             => ['attr'=>'','close'=>1],
    ];
    /*
    * 自定义导航
    */
   public function tagNavcats($tag,$content)
   {
     $item  = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
     $returnVarName = 'categories_data';
     $categoryId = "0";
     ...
     $navId=empty($tag['navId']) ? "1" : $tag['navId'];
    $parse = <<<parse
<?php
\${$returnVarName} =(new \app\admin\model\NavMenuModel())->getCategories({$categoryId},{$navId});
...
parse;
    ...
    public function tagLinks($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
     \$__LINKS__ = \app\admin\service\ApiService::links({$id});
?>
