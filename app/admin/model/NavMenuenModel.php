...
class NavMenuenModel extends Model
{
      /*
      * 自定义导航菜单获取数据
      */
      public  function getCategories($categoryId,$navId)
      {
        $navMenus=static::where(['parent_id'=>$categoryId,'nav_id'=>$navId,'status'=>1])
    ...
    public function selectNavs()
    {
        ...
        // 处理单页 分类 入库 2018.10.29
          foreach($navs as $key=>&$navData){
            foreach($navData['items'] as &$v){
              if($navData['rule']['action'] == 'portal/Listen/index'){
                $v['catid']=$v['id'];
              }else{
                $v['catid']=cmf_get_catiden_by_pageid($v['id'])['category_id'];
              }
            }
        ...
        // -------------------------------------
          ...
          foreach ($navs as $key => &$navData) {
              ...
              $tpl                = "<option value='\$rule' data-name='\$name' data-catid='\$catid'>\$spacer\$name</option>";
