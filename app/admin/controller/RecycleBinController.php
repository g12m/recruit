...
class RecycleBinController extends AdminBaseController
{
    ...
    function restore()
    {
      ...
      //多选
      if (isset($ids)) {
        $result = Db::name('recycle_bin')->where(['id' => ['in', $ids]])->select()->toArray();
        foreach ($result as $key => $val) {
          $tableName = explode('#', $val['table_name']);
          $tableName = $tableName[0];
          //还原资料
          if ($result) {
            $res = Db::name($tableName)->where(['id' => $val['object_id']])->update(['delete_time' => '0']);
            if ($tableName == 'portal_post') {
              Db::name('portal_category_post')->where('post_id',$val['object_id'])->update(['status'=>1]);
              Db::name('portal_tag_post')->where('post_id',$val['object_id'])->update(['status'=>1]);
            }
            $res = Db::name('recycle_bin')->where('id',$val['id'])->delete();
          }
        }
        $this->success("还原成功！");
     ...
     function delete()
         {
             ...
             //多选
          if (isset($ids)) {
              $result = Db::name('recycleBin')->where(['id' => ['in', $ids]])->select()->toArray();
               foreach ($result as $key => $val) {
                 //删除资源
                 if ($result) {
                     //页面没有单独的表.
                     if($val['table_name'] === 'portal_post#page'){
                         $re = Db::name('portal_post')->where('id', $val['object_id'])->delete();
                         //消除路由
                         $routeModel = new RouteModel();
                         $routeModel->setRoute('', 'portal/Page/index', ['id' => $val['object_id']], 2, 5000);
                         $routeModel->getRoutes(true);
                     }else{
                         $re = Db::name($val['table_name'])->where('id', $val['object_id'])->delete();
                     }

                     if ($re) {
                         $res = Db::name('recycleBin')->where('id', $val['id'])->delete();
                         if($val['table_name'] === 'portal_post'){
                             Db::name('portal_category_post')->where('post_id',$val['object_id'])->delete();
                             Db::name('portal_tag_post')->where('post_id',$val['object_id'])->delete();
                         }
                     }
                 }
               }
