    ...
    public function adminCategoryTableTree($currentIds = 0, $tpl = '',$hideId = 0)
    {
       ...
       if (!empty($hideId)) {
        ...
        foreach ($categories as $key=> $item) {
            ...
            $categories[$key]['str_action'] = '<a href="' . url("AdminCategory/add", ["parent" => $item['id']]) . '">添加子分类</a>  <a href="' . url("AdminCategory/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="js-ajax-delete" href="' . url("AdminCategory/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            $categories[$key]['checked'] = in_array($item['id'], $currentIds) ? "checked" : "";
            $categories[$key]['url']     = cmf_url('portal/List/index', ['id' => $item['id']]);;
            $categories[$key]['status_text']    = empty($item['status'])?'隐藏':'显示';
            if ($item['status']) {
              $categories[$key]['str_action'] .= '<a class="js-ajax-dialog-btn" data-msg="您确定隐藏此分类吗" href="' . url('AdminCategory/toggle', ['ids' => $item['id'], 'hide' => 1]) . '">隐藏</a>';
                      } else {
              $categories[$key]['str_action'] .= '<a class="js-ajax-dialog-btn" data-msg="您确定显示此分类吗" href="' . url('AdminCategory/toggle', ['ids' => $item['id'], 'display' => 1]) . '">显示</a>';
            }
            // array_push($newCategories, $item);
        }
