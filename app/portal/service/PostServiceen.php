...
class PostServiceen
{
    ...
    public function adminPostList($filter,$currentCid,$isPage = false)
    {
      ...
      // return $currentCid;
        ...
        if (!empty($category)) {
            ...
            array_push($join, [
                '__PORTAL_CATEGORYEN_POST__ b', 'a.id = b.post_id'
            ]);
    ...
    public function publishedArticle($postId, $categoryId = 0)
    {
        ...
        if (empty($categoryId)) {
            ...
            $join    = [
                ['__PORTAL_CATEGORYEN_POST__ relation', 'post.id = relation.post_id']
            ];
            $article = $portalPostModel->alias('post')->field('post.*')
                ->join($join)
                ->where($where)
    ...
    public function publishedPage($pageId)
    {
        ...
        $page            = $portalPostModel
            ->where($where)
            ->alias('post')
            ->join('__PORTAL_CATEGORYEN_POST__ category_post','post.id = category_post.post_id')
