...
class AssetController extends AdminBaseController
{
    ...
    public function webuploader()
    {
        if ($this->request->isPost()) {
            ...
            // 处理水印
            if($arrData['filetype'] == 'image'){
              $mark_setting   =  cmf_get_option('mark_setting');
              if($mark_setting['is_mark'] == 1){
                $mark= $mark_setting['mark'] ? $mark_setting['mark'] : "mark.png";
                // 裁剪水印图片
                $thumb_image = \think\Image::open('./upload/mark/'.$mark);
                // 按照原图的比例生成一个最大为  * 的缩略图并保存为thumb.png
                $thumb_image->thumb($mark_setting['width'],$mark_setting['height'],\think\Image::THUMB_CENTER)->save('./upload/mark/'.$mark);
                // 添加水印图片
                $mark= $mark_setting['mark'] ? $mark_setting['mark'] : "mark.png";
                $filepath=$result['url'];
                $image = \think\Image::open('.'.$filepath);
                $image->water('./upload/mark/'.$mark,$mark_setting['location'],$mark_setting['transparency'])->save('.'.$filepath);
