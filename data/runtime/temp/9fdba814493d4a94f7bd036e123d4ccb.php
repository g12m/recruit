<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:47:"themes/admin/portal\admin_productsen\index.html";i:1546653062;s:70:"E:\phpStudy\PHPTutorial\WWW\xsj\public\themes\admin\public\header.html";i:1543480324;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="wrap js-check-wrap">
    <ul class="nav nav-tabs">
        ...
        <li><a href="<?php echo url('AdminProductsen/add'); ?>">添加文章</a></li>
    </ul>
    <form class="well form-inline margin-top-20" method="post" action="<?php echo url('AdminProductsen/index'); ?>">
        分类:
        ...
        <select class="form-control" name="status" style="width: 140px;">
            ...
            <option value='1' <?php if(($status == '1')): ?>selected<?php endif; ?>>置顶</option>
            <option value='2' <?php if(($status == '2')): ?>selected<?php endif; ?>>推荐</option>
        ...
        <a class="btn btn-danger" href="<?php echo url('AdminProductsen/index'); ?>">清空</a>
    </form>
        ...
        <div class="table-actions">
            <?php if(!(empty($category) || (($category instanceof \think\Collection || $category instanceof \think\Paginator ) && $category->isEmpty()))): ?>
                <button class="btn btn-primary btn-sm js-ajax-submit" type="submit"
                        data-action="<?php echo url('AdminProductsen/listOrder'); ?>"><?php echo lang('SORT'); ?>
                </button>
            <?php endif; ?>
            <button class="btn btn-primary btn-sm js-ajax-submit" type="submit"
                    data-action="<?php echo url('AdminProductsen/publish',array('yes'=>1)); ?>" data-subcheck="true">发布
            </button>
            <button class="btn btn-primary btn-sm js-ajax-submit" type="submit"
                    data-action="<?php echo url('AdminProductsen/publish',array('no'=>1)); ?>" data-subcheck="true">取消发布
            </button>
            <button class="btn btn-primary btn-sm js-ajax-submit" type="submit"
                    data-action="<?php echo url('AdminProductsen/top',array('yes'=>1)); ?>" data-subcheck="true">推荐
            </button>
            <button class="btn btn-primary btn-sm js-ajax-submit" type="submit"
                    data-action="<?php echo url('AdminProductsen/top',array('no'=>1)); ?>" data-subcheck="true">取消推荐
            </button>
            <!-- <button class="btn btn-primary btn-sm js-ajax-submit" type="submit"
                    data-action="<?php echo url('AdminProductsen/recommend',array('yes'=>1)); ?>" data-subcheck="true">推荐
            </button>
            <button class="btn btn-primary btn-sm js-ajax-submit" type="submit"
                    data-action="<?php echo url('AdminProductsen/recommend',array('no'=>1)); ?>" data-subcheck="true">取消推荐
            </button> -->
            <!--
            <?php if(!(empty($category) || (($category instanceof \think\Collection || $category instanceof \think\Paginator ) && $category->isEmpty()))): ?>
            ...
            <button class="btn btn-danger btn-sm js-ajax-submit" type="submit"
                    data-action="<?php echo url('AdminProductsen/delete'); ?>" data-subcheck="true" data-msg="您确定删除吗？">
        ...
        <table class="table table-hover table-bordered table-list">
            ...
            <?php if(is_array($articles) || $articles instanceof \think\Collection || $articles instanceof \think\Paginator): if( count($articles)==0 ) : echo "" ;else: foreach($articles as $key=>$vo): ?>
                <tr>
                    ...
                    <td>
                        <?php if(!(empty($category) || (($category instanceof \think\Collection || $category instanceof \think\Paginator ) && $category->isEmpty()))): ?>
                            <a href="<?php echo cmf_url('portal/Articleen/index',array('id'=>$vo['id'],'cid'=>$vo['category_id'])); ?>"
                               target="_blank"><?php echo $vo['post_title']; ?></a>
                            <?php else: ?>
                            <a href="<?php echo cmf_url('portal/Articleen/index',array('id'=>$vo['id'])); ?>"
                               target="_blank"><?php echo $vo['post_title']; ?></a>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php if(is_array($vo['categories']) || $vo['categories'] instanceof \think\Collection || $vo['categories'] instanceof \think\Paginator): if( count($vo['categories'])==0 ) : echo "" ;else: foreach($vo['categories'] as $key=>$voo): ?>
                            <span class="label label-default">
                                <a href="<?php echo cmf_url('portal/Listen/index',array('id'=>$voo['id'])); ?>"
                                   style="color: #fff;"
                                   target="_blank"
                                ><?php echo $voo['name']; ?></a>
                            </span>&nbsp;
                    ...
                    <td>
                        <a href="<?php echo url('AdminProductsen/edit',array('id'=>$vo['id'])); ?>"><?php echo lang('EDIT'); ?></a>
                        <a href="<?php echo url('AdminProductsen/delete',array('id'=>$vo['id'])); ?>" class="js-ajax-delete"><?php echo lang('DELETE'); ?></a>
                    </td>
                </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
    ...
    $(function () {
        ...
        Wind.use('ajaxForm', 'artDialog', 'iframeTools', function () {
            ...
            $('.js-articles-copy').click(function (e) {
                ...
                ids = ids.join(',');
                art.dialog.open("/index.php?g=portal&m=AdminProductsen&a=copy&ids=" + ids, {
                    title: "批量复制",
                    width: "300px"
                });
            });
            ...
            $('.js-articles-move').click(function (e) {
                ...
                ids = ids.join(',');
                art.dialog.open("/index.php?g=portal&m=AdminProductsen&a=move&old_term_id=<?php echo (isset($term['term_id']) && ($term['term_id'] !== '')?$term['term_id']:0); ?>&ids=" + ids, {
                    title: "批量移动",
                    width: "300px"
                });
            });
